...
namespace VulkanCore {
    ...
    class Context {
    public:
        static std::expected<Context, std::string> create(
            GLFWwindow *window,
            ...
            Context context{
                window,
                applicationName,
                std::move(requestedInstanceLayer),
                std::move(requestedInstanceExtensions)
            };
    ...
    private:
        Context(
            ...
            m_layersView = m_requestedInstanceLayer
                           | views::transform(std::mem_fn(&std::string::c_str))
                           | ranges::to<std::vector<const char *>>();
            m_extensionsView = m_requestedInstanceExtensions
                               | views::transform(std::mem_fn(&std::string::c_str))
                               | ranges::to<std::vector<const char *>>();
        }
        ...
        bool init() {
            ...
            const VkInstanceCreateInfo instanceCreateInfo{
                ...
                .enabledExtensionCount = static_cast<uint32_t>(m_extensionsView.size()),
...
int main() {
    ...
    auto vulkanContext = VulkanCore::Context::create(
        window,
        applicationName,
